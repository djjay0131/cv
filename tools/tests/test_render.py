...
def test_publications_default_to_early_placement(tmp_path: Path) -> None:
    content_dir = _setup_content(tmp_path)
    variants_dir = _setup_variant(tmp_path)
    templates_dir = _setup_templates(tmp_path)
    output_dir = tmp_path / "out"
    ...
    theme = (output_dir / "_theme.tex").read_text()
    assert r"\def\cvpubslate{false}" in theme


def test_publications_late_flag_emitted(tmp_path: Path) -> None:
    content_dir = _setup_content(tmp_path)
    variants_dir = _setup_variant(tmp_path, overrides="publications_late: true\n")
    templates_dir = _setup_templates(tmp_path)
    output_dir = tmp_path / "out"
    ...
    theme = (output_dir / "_theme.tex").read_text()
    assert r"\def\cvpubslate{true}" in theme
